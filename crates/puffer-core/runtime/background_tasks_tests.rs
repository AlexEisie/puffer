...
fn auto_background_inline_fast_task() {
    let result = run_with_auto_background(
        "fast task",
...
fn head_tail_buffer_many_small_writes_simulating_streaming() {
    ...
    assert!(output.contains("bytes truncated"), "should have truncation marker");
}
...
fn task_manager_concurrent_thread_writes_to_buffer() {
    let mgr = BackgroundTaskManager::new();
    let buf = mgr.register("mt-1", "multi-thread test", None, None, false).unwrap();
...
fn auto_background_slow_task_registered_in_manager() {
    ...
    match result {
        AutoBgResult::Backgrounded { task_id, .. } => {
            ...
            assert!(info.is_some(), "auto-backgrounded task should be in manager");
...
fn end_to_end_background_task_lifecycle() {
    ...
    let buf = mgr
        .register("e2e-agent-1", "explore codebase", Some("agent-001"), None, false)
    ...
    assert!(partial.contains("Turn 1"), "should have at least first turn");
