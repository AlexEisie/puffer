...
impl ChatImageOutputAdapter {
    ...
    /// Executes an exact chat image-output request and persists job/artifact sidecars.
    pub(crate) fn execute(
        ...
        self.execute_with_discovery_cache(
            registry,
            auth_store,
            service,
            request,
...
fn download_image_url(client: &Client, url: &str) -> Result<Vec<u8>> {
    let parsed = reqwest::Url::parse(url).context("chat image-output URL must be absolute")?;
    match parsed.scheme() {
        "https" => {}
        "http" if url_host_is_loopback(&parsed) => {}
        other => bail!("unsupported chat image-output URL scheme `{other}`"),
    }
    let response = client
        .get(parsed)
        .send()
        .context("download chat image-output image")?;
    let status = response.status();
    if !status.is_success() {
        bail!(
            "download chat image-output image failed with status {}",
            status.as_u16()
        );
    }
    Ok(response
        .bytes()
        .context("read chat image-output image bytes")?
        .to_vec())
}
...
fn provider_execution_url(
    ...
    let mut url = reqwest::Url::parse(&base)
        ...
        .with_context(|| {
            format!(
                "build chat image-output URL from {} and {}",
...
fn provider_credential<'a>(
    ...
    auth_store.get(&provider.id).or_else(|| {
