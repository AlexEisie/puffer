...
mod tests {
    ...
    fn transcript_to_items_empty_state() {
        ...
        assert!(matches!(&items[0], ConversationItem::Message { role, content, .. }
    ...
    fn transcript_to_items_with_history() {
        ...
        assert!(matches!(&items[1], ConversationItem::Message { role, id, .. }
            if role == "assistant" && id.is_some()));
        assert!(matches!(&items[2], ConversationItem::Message { role, content, .. }
    ...
    fn append_tool_results_adds_items() {
        ...
        assert!(matches!(&items[2], ConversationItem::FunctionCallOutput { output, .. } if output == "result"));
    }
