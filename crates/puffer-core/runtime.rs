...
fn transcript_to_openai_input(state: &AppState, input: &str) -> Result<Value> {
    let plan_mode_context = crate::command_helpers::prompt::plan_mode_context_message(state)?;
...
fn transcript_to_openai_chat_messages(state: &AppState, input: &str) -> Result<Vec<OpenAIChatMessage>> {
    let plan_mode_context = crate::command_helpers::prompt::plan_mode_context_message(state)?;
